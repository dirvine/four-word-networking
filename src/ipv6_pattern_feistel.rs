...
impl IPv6PatternId {
    ...
    pub fn from_ipv6_pattern(pattern: &crate::ipv6_perfect_patterns::IPv6Pattern) -> Self {
        ...
        match pattern {
            ...
            IPv6Pattern::CloudProvider(_) | IPv6Pattern::CommonProvider(_) | IPv6Pattern::Unstructured => IPv6PatternId::Other,
        }
    }
}
...
impl IPv6PatternFeistel {
    ...
    fn encode_small(pattern_bits: u64, data: u64, data_bits: usize) -> Result<u64> {
        ...
        result |= (pattern_mask & 1) << 0;        // A at bit 0
...
mod tests {
    ...
    fn test_small_data_roundtrip() {
        ...
        for (pattern_id, data, bits) in test_cases {
            ...
            let (decoded_pattern, decoded_data) = IPv6PatternFeistel::decode(encoded, bits).unwrap();
    ...
    fn test_large_data_roundtrip() {
        ...
        for (pattern_id, data, bits) in test_cases {
            println!("Testing pattern {:?} with data 0x{:X} ({} bits)", pattern_id, data, bits);
            
            let encoded = IPv6PatternFeistel::encode(pattern_id, data, bits).unwrap();
            println!("  Encoded: 0x{:X}", encoded);
            
            let (decoded_pattern, decoded_data) = IPv6PatternFeistel::decode(encoded, bits).unwrap();
            println!("  Decoded pattern: {:?}, data: 0x{:X}", decoded_pattern, decoded_data);
            
            assert_eq!(decoded_pattern, pattern_id, "Pattern mismatch for {:?}", pattern_id);
    ...
    fn test_different_patterns_produce_different_output() {
        ...
        let encoded2 = IPv6PatternFeistel::encode(IPv6PatternId::Documentation, data, bits).unwrap();
        
        assert_ne!(encoded1, encoded2, "Different patterns should produce different encoded data");
    }
