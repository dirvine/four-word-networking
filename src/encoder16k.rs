...
impl UniversalEncoder16K {
    ...
    fn decode_hybrid(&self, words: &[String; 3], digits: &[String]) -> Result<Vec<u8>, DecodingError> {
        ...
        for digit_group in digits {
            let value: u16 = digit_group.parse()
                .map_err(|_| DecodingError::InvalidFormat(format!("Invalid digit group: {}", digit_group)))?;
...
mod tests {
    ...
    fn test_hybrid_encoding() {
        ...
        match encoded {
            Encoding16K::Hybrid { words, digits } => {
                ...
                println!("IPv6 encoding: {} + {} digits", words.join("."), digits.len() * 4);
            }
    ...
    fn test_bitcoin_address_encoding() {
        ...
        match encoded {
            Encoding16K::Hybrid { words, digits } => {
                ...
                assert!(digits.len() <= 8);
                println!("Bitcoin encoding: {} + {} digits", words.join("."), digits.len() * 4);
            }
    ...
    fn test_efficiency_info() {
        ...
        for (data, expected_rating) in test_cases {
            ...
            println!("{} bytes: {} ({})", data.len(), efficiency.description, efficiency.efficiency_rating);
        }
    }
    ...
    fn test_error_conditions() {
        ...
        let invalid_encoding = Encoding16K::Simple {
            words: ["nonexistent".to_string(), "word".to_string(), "here".to_string()],
        };
