...
impl IPv6MultiDimEncoding {
    ...
    pub fn to_string(&self) -> String {
        ...
        for (i, &word_idx) in word_indices.iter().enumerate() {
            if word_idx < self.words.len() {
                ...
                let case_pattern = self.case_patterns.get(word_idx).unwrap_or(&CasePattern::Lower);
    ...
    pub fn from_string(s: &str, dictionary: &IPv6Dictionary) -> Result<Self> {
        ...
        if parts.len() < 4 || parts.len() > 6 {
            return Err(FourWordError::InvalidInput(
                format!("IPv6 requires 4-6 words, found {}", parts.len())
            ));
        }
    ...
    fn analyze_case_pattern(word: &str) -> (String, CasePattern) {
        ...
        let alternating = normalized.chars().enumerate().map(|(i, c)| {
            if i % 2 == 0 {
                c.to_uppercase().collect::<String>()
            } else {
                c.to_lowercase().collect::<String>()
        ...
        let reverse_alternating = normalized.chars().enumerate().map(|(i, c)| {
            if i % 2 == 0 {
                c.to_lowercase().collect::<String>()
            } else {
                c.to_uppercase().collect::<String>()
    ...
    fn detect_word_order(words: &[String], dictionary: &IPv6Dictionary) -> Result<Option<Vec<usize>>> {
...
impl CasePattern {
    ...
    pub fn from_bits(bits: u8) -> Result<Self> {
        match bits {
            ...
            _ => Err(FourWordError::InvalidInput(format!("Invalid case pattern bits: {}", bits))),
        }
    }
...
impl Separator {
    ...
    pub fn from_bits(bits: u8) -> Result<Self> {
        match bits {
            ...
            _ => Err(FourWordError::InvalidInput(format!("Invalid separator bits: {}", bits))),
        }
    }
...
impl IPv6MultiDimEncoder {
    ...
    pub fn encode(&self, data: u64, extra_bits: Option<u64>, word_count: usize) -> Result<IPv6MultiDimEncoding> {
    ...
    fn encode_with_extensions(&self, data: u64, extra_bits: Option<u64>, word_count: usize) -> Result<IPv6MultiDimEncoding> {
        let mut words = Vec::new();
        let mut case_patterns = Vec::new();
        let mut separators = Vec::new();
        let mut remaining_data = data;
        ...
        let total_extra_bits = case_bits_available + separator_bits_available + order_bits_available;
    ...
    pub fn decode(&self, encoding: &IPv6MultiDimEncoding) -> Result<(u64, Option<u64>)> {
        ...
        for (i, &word_idx) in word_order.iter().enumerate() {
            if let Some(word_index) = self.dictionary.find_word(&encoding.words[word_idx]) {
                ...
                return Err(FourWordError::InvalidInput(
                    format!("Word not found: {}", encoding.words[word_idx])
                ));
            }
        }
        ...
        if let Some(ref order) = encoding.word_order {
            let order_bits = Self::permutation_bits(encoding.words.len());
            let order_value = Self::encode_permutation(order)?;
            extra_data |= (order_value as u64) << sep_bit_offset;
        }
        
        Ok((data, if extra_data > 0 { Some(extra_data) } else { None }))
    }
    ...
    fn encode_permutation(permutation: &[usize]) -> Result<usize> {
        ...
        for &p in permutation {
            if let Some(pos) = available.iter().position(|&x| x == p) {
                ...
                return Err(FourWordError::InvalidInput(
                    format!("Invalid permutation element: {}", p)
                ));
            }
        }
...
mod tests {
    ...
    fn test_case_pattern_analysis() {
        assert_eq!(IPv6MultiDimEncoding::analyze_case_pattern("hello"), ("hello".to_string(), CasePattern::Lower));
        assert_eq!(IPv6MultiDimEncoding::analyze_case_pattern("HELLO"), ("hello".to_string(), CasePattern::Upper));
        assert_eq!(IPv6MultiDimEncoding::analyze_case_pattern("Hello"), ("hello".to_string(), CasePattern::Title));
    }
