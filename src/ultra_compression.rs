...
impl UltraCompressor {
    ...
    pub fn ultra_compress(&self, multiaddr: &str) -> Result<Vec<u8>, UltraCompressionError> {
        ...
        match ip_protocol {
            ...
            _ => Err(UltraCompressionError::UnsupportedPattern(
                multiaddr.to_string(),
            )),
        }
    }
    ...
    fn compress_ipv4(
        ...
        protocol: &str,
        port: u16,
    ) -> Result<Vec<u8>, UltraCompressionError> {
    ...
    pub fn ultra_decompress(&self, data: &[u8]) -> Result<String, UltraCompressionError> {
        if data.is_empty() {
            return Err(UltraCompressionError::InvalidFormat(
        ...
        match category {
            ...
            _ => Err(UltraCompressionError::InvalidFormat(format!(
    ...
    fn decode_localhost(
        ...
        protocol: &str,
        is_ipv6: bool,
    ) -> Result<String, UltraCompressionError> {
        ...
        Ok(format!(
            "/{}/{}/{}/{}",
            ip_protocol, ip_addr, protocol, port
        ))
    }

    /// Decode private network patterns  
    fn decode_private_network(
        &self,
        data: &[u8],
        protocol: &str,
    ) -> Result<String, UltraCompressionError> {
        if data.is_empty() {
            return Err(UltraCompressionError::InvalidFormat(
                "Empty private network data".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_ultra_compression_localhost() {
        ...
        let result = compressor
            .ultra_compress("/ip4/127.0.0.1/tcp/4001")
            .unwrap();
        assert!(result.len() <= 5);
        assert!(result.len() >= 3);
    ...
    fn test_ultra_compression_private_network() {
        ...
        let result = compressor
            .ultra_compress("/ip4/192.168.1.100/tcp/80")
            .unwrap();
        assert!(result.len() <= 5);

        println!(
            "Private network compressed to {} bytes: {:?}",
            result.len(),
            result
        );
    }
