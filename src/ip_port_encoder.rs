...
impl CompressedIpPort {
    ...
    pub fn compress(addr: &IpPortAddress) -> Result<Self, FourWordError> {
                    ...
                    if let Some(port) = addr.port {
                        ...
                        if let Some(&(_, code)) = COMMON_PORTS.iter().find(|&&(p, _)| p == port) {
                            ...
                            return Err(FourWordError::InvalidInput(
                                "10.x.x.x addresses with non-common ports exceed 5 bytes"
                                    .to_string(),
                            ));
                        }
                    }
                    ...
                    if let Some(port) = addr.port {
                        ...
                        if let Some(&(_, code)) = COMMON_PORTS.iter().find(|&&(p, _)| p == port) {
                            ...
                            if code > 0x0F {
                                ...
                                return Err(FourWordError::InvalidInput(
                                    "172.16-31.x.x addresses require smaller port codes"
                                        .to_string(),
                                ));
                            }
                            ...
                            return Err(FourWordError::InvalidInput(
                                "172.16-31.x.x addresses with non-common ports exceed 5 bytes"
                                    .to_string(),
                            ));
                        }
                    }
        ...
        if data.len() > MAX_COMPRESSED_SIZE {
            return Err(FourWordError::InvalidInput(format!(
                "Compressed size {} exceeds maximum {}",
                data.len(),
                MAX_COMPRESSED_SIZE
            )));
        }
        ...
        match header {
            ...
            0x81 => {
                ...
                if idx + 1 >= self.data.len() {
                    return Err(FourWordError::InvalidInput(
                        "Incomplete 192.168.x.x address".to_string(),
                    ));
                }
            ...
            0x82 => {
                ...
                if idx + 2 >= self.data.len() {
                    return Err(FourWordError::InvalidInput(
                        "Incomplete 10.x.x.x address".to_string(),
                    ));
                }
            ...
            0x83 => {
                ...
                if idx + 2 >= self.data.len() {
                    return Err(FourWordError::InvalidInput(
                        "Incomplete 172.16-31.x.x address".to_string(),
                    ));
                }
            ...
            0x84 => {
                ...
                if idx + 3 >= self.data.len() {
                    return Err(FourWordError::InvalidInput(
                        "Incomplete public IPv4 address".to_string(),
                    ));
                }
    ...
    fn decompress_port(&self, idx: &mut usize) -> Result<Option<u16>, FourWordError> {
        ...
        if *idx + 1 >= self.data.len() {
            return Err(FourWordError::InvalidInput(
                "Incomplete port data".to_string(),
            ));
        }
