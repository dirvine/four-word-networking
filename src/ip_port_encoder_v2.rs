...
impl IpPortCompressor {
    ...
    fn encode_ipv4_private_10(&self, second: u8, third: u8, fourth: u8, port: u16) -> Result<Vec<u8>> {
        ...
        if let Some(&port_code) = self.common_ports.get(&port) {
            ...
            Ok(vec![0x31, (hash >> 8) as u8, hash as u8, (port >> 8) as u8, port as u8])
        }
    }
    
    /// Encode IPv4 172.16-31.x.x (4-5 bytes)
    fn encode_ipv4_private_172(&self, second: u8, third: u8, fourth: u8, port: u16) -> Result<Vec<u8>> {
        let hash = ((second - 16) as u16) | ((third as u16) << 4) | ((fourth as u16) << 12);
        
        if let Some(&port_code) = self.common_ports.get(&port) {
            Ok(vec![0x40, (hash >> 8) as u8, hash as u8, port_code])
        } else {
            Ok(vec![0x41, (hash >> 8) as u8, hash as u8, (port >> 8) as u8, port as u8])
        }
    }
    ...
    fn encode_ipv4_public(&self, octets: &[u8; 4], port: u16) -> Result<Vec<u8>> {
        ...
        let addr_hash = (octets[0] as u16) ^ ((octets[1] as u16) << 2) 
                      ^ ((octets[2] as u16) << 4) ^ ((octets[3] as u16) << 6);
    ...
    fn decompress(&self, data: &[u8]) -> Result<SocketAddr> {
        ...
        match header {
            ...
            _ => Err(IpPortErrorV2::DecompressionError(
                format!("Unknown header: 0x{:02x}", header)
            )),
        }
    }
    ...
    fn decode_ipv4_localhost(&self, data: &[u8]) -> Result<SocketAddr> {
        if data.len() < 3 {
            return Err(IpPortErrorV2::DecompressionError("Insufficient data".to_string()));
        }
    ...
    fn test_round_trip_encoding() {
        ...
        for original in test_cases {
            ...
            if original.contains("192.168.") && decoded.contains("192.168.") {
                let orig_port = original.split(':').last().unwrap();
                let dec_port = decoded.split(':').last().unwrap();
                assert_eq!(orig_port, dec_port, "Ports should match for private network");
            }
