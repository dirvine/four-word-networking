...
use crate::universal::error::{EncodingError, DecodingError};
...
impl Dictionaries {
    ...
    pub fn new() -> Result<Self, EncodingError> {
        ...
        if actors.len() != 4096 || actions.len() != 4096 || 
           objects.len() != 4096 || modifiers.len() != 4096 {
            return Err(EncodingError::DictionaryNotLoaded);
        }
        
        // Create reverse lookup maps
        let actor_indices: HashMap<String, usize> = actors.iter()
            .enumerate()
            .map(|(i, word)| (word.clone(), i))
            .collect();
        
        let action_indices: HashMap<String, usize> = actions.iter()
            .enumerate()
            .map(|(i, word)| (word.clone(), i))
            .collect();
        
        let object_indices: HashMap<String, usize> = objects.iter()
            .enumerate()
            .map(|(i, word)| (word.clone(), i))
            .collect();
        
        let modifier_indices: HashMap<String, usize> = modifiers.iter()
            .enumerate()
            .map(|(i, word)| (word.clone(), i))
            .collect();
    ...
    pub fn get_modifier(&self, index: usize) -> Result<&str, DecodingError> {
        self.modifiers.get(index)
            .map(|s| s.as_str())
            .ok_or_else(|| DecodingError::DictionaryLookupFailed(format!("modifier index {}", index)))
    }
    
    /// Get actor index by word
    pub fn get_actor_index(&self, word: &str) -> Result<usize, DecodingError> {
        self.actor_indices.get(word)
            .copied()
    ...
    pub fn get_modifier_index(&self, word: &str) -> Result<usize, DecodingError> {
        self.modifier_indices.get(word)
            .copied()
            .ok_or_else(|| DecodingError::InvalidWord(word.to_string()))
    }
    ...
    fn generate_unique_words(base_words: &[&str]) -> Vec<String> {
        ...
        while words.len() < 4096 {
            ...
            while seen.contains(&final_word) {
                counter += 1;
                final_word = format!("{}z{}", &base[..std::cmp::min(base.len(), 4)], (i + counter) % 1000);
            }
    ...
    fn load_actors() -> Vec<String> {
        let base_actors = vec![
            "falcon", "wizard", "knight", "dragon", "phoenix", "archer", "warrior", "mage",
            "ranger", "paladin", "rogue", "cleric", "monk", "sorcerer", "warlock", "druid",
            "barbarian", "bard", "fighter", "thief", "assassin", "hunter", "guardian", "champion",
            "hero", "legend", "mystic", "sage", "oracle", "prophet", "vision", "spirit",
            "giant", "demon", "angel", "beast", "ghost", "shade", "wraith", "specter",
            "titan", "golem", "dwarf", "gnome", "pixie", "fairy", "sprite", "nymph",
            "witch", "shaman", "priest", "monk2", "ninja", "samurai", "ronin", "shogun",
            "viking", "berserker", "centurion", "gladiator", "spartan", "amazon", "valkyrie", "banshee",
        ];
    ...
    fn load_actions() -> Vec<String> {
        let base_actions = vec![
            "crosses", "guards", "seeks", "finds", "hunts", "builds", "climbs", "flies",
            "walks", "runs", "jumps", "swims", "dives", "soars", "glides", "dances",
            "sings", "plays", "reads", "writes", "draws", "paints", "crafts", "creates",
            "explores", "discovers", "learns", "teaches", "helps", "protects", "fights", "defends",
            "attacks", "blocks", "dodges", "parries", "strikes", "casts", "summons", "banishes",
            "heals", "curses", "blesses", "enchants", "forges", "melts", "freezes", "burns",
            "lights", "darkens", "opens", "closes", "locks", "unlocks", "breaks", "repairs",
            "grows", "shrinks", "expands", "contracts", "lifts", "drops", "pushes", "pulls",
        ];
    ...
    fn load_objects() -> Vec<String> {
        let base_objects = vec![
            ...
            "lake", "valley", "cave", "temple", "palace", "garden", "field", "meadow",
            "stone", "crystal", "gem", "sword", "shield", "armor", "crown", "ring",
            "book", "scroll", "map", "key", "door", "gate", "path", "road",
            "staff", "wand", "orb", "amulet", "potion", "spell", "charm", "curse",
            "flame", "frost", "storm", "earth", "wind", "water", "fire", "light",
            "shadow", "void", "energy", "power", "force", "magic", "spirit", "soul",
    ...
    fn load_modifiers() -> Vec<String> {
        let base_modifiers = vec![
            ...
            "bright", "dark", "light", "heavy", "quick", "slow", "strong", "weak",
            "large", "small", "tall", "short", "wide", "narrow", "deep", "shallow",
            "hot", "cold", "warm", "cool", "dry", "wet", "rough", "smooth",
            "sharp", "dull", "soft", "hard", "young", "old", "new", "worn",
            "pure", "corrupt", "holy", "cursed", "blessed", "damned", "sacred", "profane",
            "mighty", "frail", "bold", "timid", "brave", "coward", "wise", "foolish",
            "noble", "common", "rare", "unique", "special", "normal", "strange", "weird",
        ];
...
mod tests {
    ...
    fn test_dictionaries_creation() {
        ...
        println!("Actors: {}, Actions: {}, Objects: {}, Modifiers: {}", 
                 dict.actors.len(), dict.actions.len(), dict.objects.len(), dict.modifiers.len());
        ...
        assert_eq!(dict.modifiers.len(), 4096, "Modifiers dictionary size mismatch");
    ...
    fn test_word_constraints() {
        ...
        for word in &dict.actors[..10] { // Test first 10 for efficiency
            assert!(word.len() >= 4 && word.len() <= 8, "Actor word '{}' violates length constraint", word);
        }
        
        for word in &dict.actions[..10] {
            assert!(word.len() >= 4 && word.len() <= 8, "Action word '{}' violates length constraint", word);
        }
        
        for word in &dict.objects[..10] {
            assert!(word.len() >= 4 && word.len() <= 8, "Object word '{}' violates length constraint", word);
        }
        
        for word in &dict.modifiers[..10] {
            assert!(word.len() >= 4 && word.len() <= 8, "Modifier word '{}' violates length constraint", word);
        }
    }
}