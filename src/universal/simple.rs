...
impl FourWords {
    ...
    pub fn from_string(s: &str) -> DecodingResult<Self> {
        ...
        if parts.len() != 3 {
            return Err(DecodingError::InvalidFormat(format!(
                "Expected 4 words separated by dots, got {}",
                parts.len()
            )));
        }
    ...
    fn test_word_constraints() {
        ...
        for word in encoded.all_words() {
            assert!(
                ...
                "Word '{}' violates length constraint",
                word
            );
        }
    }
