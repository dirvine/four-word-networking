...
mod tests {
    ...
    fn test_ultra_compact_1_million_addresses() {
        ...
        println!("  Average compression: {:.1}%", summary.average_compression_ratio);
        println!("  Word count range: {}-{} words", summary.min_word_count, summary.max_word_count);
        ...
        for pattern in &common_patterns {
            match encoder.encode(pattern) {
                Ok(encoded) => {
                    ...
                    println!("  {} -> {} ({} words, {:.1}% compression) {}", 
                        pattern, encoded.to_words(), encoded.word_count(), compression,
                        if is_perfect { "✅ PERFECT" } else { "📈 MULTI" });
                }
                Err(e) => {
