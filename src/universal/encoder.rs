...
use crate::universal::{
    ...
    error::{DecodingResult, EncodingError, EncodingResult},
...
impl UniversalEncoder {
    ...
    pub fn decode(&self, encoding: &UniversalEncoding) -> DecodingResult<Vec<u8>> {
        match encoding {
            UniversalEncoding::Simple(simple) => self.simple_encoder.decode(simple),
            UniversalEncoding::Fractal(fractal) => self.fractal_encoder.decode(fractal),
...
mod tests {
    ...
    fn test_strategy_selection() {
        ...
        assert_eq!(
            encoder.strategy_for_length(4).unwrap(),
            EncodingStrategy::Simple
        );
        assert_eq!(
            encoder.strategy_for_length(12).unwrap(),
            EncodingStrategy::Fractal
        );
        assert_eq!(
            encoder.strategy_for_length(28).unwrap(),
            EncodingStrategy::Holographic
        );
