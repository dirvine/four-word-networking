...
fn demo_perfect_round_trip(encoder: &IpPortEncoderV2) -> Result<(), IpPortErrorV2> {
    ...
    for original in test_addresses {
        ...
        let matches = if original.contains("10.")
            || original.contains("172.")
            || original.contains("[fe80")
            || original.contains("[2001")
        {
...
fn demo_all_address_types(encoder: &IpPortEncoderV2) -> Result<(), IpPortErrorV2> {
    ...
    let categories = vec![
        (
            "IPv4 Localhost",
            vec!["127.0.0.1:80", "127.0.0.1:443", "127.0.0.1:22"],
        ),
        (
            "IPv4 Private 192.168",
            vec![
                "192.168.1.1:80",
                "192.168.1.100:8080",
                "192.168.100.200:443",
        ...
        (
            "IPv4 Private 10.x",
            vec!["10.0.0.1:22", "10.10.10.10:3306", "10.255.255.255:9000"],
        ),
        (
            "IPv4 Private 172.x",
            vec!["172.16.0.1:80", "172.16.10.20:443", "172.31.255.255:8080"],
        ),
        (
            "IPv6 Localhost",
            vec!["[::1]:80", "[::1]:443", "[::1]:8080"],
        ),
        (
            "IPv6 Link-Local",
            vec!["[fe80::1]:80", "[fe80::1234:5678]:443"],
        ),
        (
            "IPv6 Global",
            vec!["[2001:db8::1]:80", "[2001:db8::8a2e:370:7334]:443"],
        ),
    ];
...
fn demo_compression_stats(encoder: &IpPortEncoderV2) -> Result<(), IpPortErrorV2> {
    ...
    let test_cases = vec![
        (
            "IPv4 Localhost",
            vec!["127.0.0.1:80", "127.0.0.1:443", "127.255.255.255:65535"],
        ),
        (
            "IPv4 Private",
            vec!["192.168.1.1:80", "10.0.0.1:80", "172.16.0.1:80"],
        ),
        (
            "IPv6 Localhost",
            vec!["[::1]:80", "[::1]:443", "[::1]:8080"],
    ...
    println!(
        "{:<20} | {:<15} | {:<15} | {:<15}",
        "Category", "Avg Compression", "Min", "Max"
    );
    println!("{}", "-".repeat(70));
    ...
    for (category, addresses) in test_cases {
        ...
        if !compressions.is_empty() {
            ...
            let min = compressions
                .iter()
                .min_by(|a, b| a.partial_cmp(b).unwrap())
                .unwrap();
            let max = compressions
                .iter()
