...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if reordered_words.len() != 16384 {
        println!(
            "⚠️  Warning: Reordered dictionary has {} words, expected 16384",
            reordered_words.len()
        );
    ...
    println!(
        "   High quality words are now at indices 0-{}",
        high_quality.len() - 1
    );
    println!(
        "   Medium quality words at indices {}-{}",
        high_quality.len(),
        high_quality.len() + medium_quality.len() - 1
    );
    println!(
        "   Low quality words at indices {}-16383",
        high_quality.len() + medium_quality.len()
    );
...
fn assess_word_quality(word: &str) -> WordQuality {
    ...
    let low_quality_words = [
        "aim", "bok", "cod", "dab", "duh", "duo", "eel", "emu", "fax", "gab", "gag", "keg", "nag",
        "oaf", "pox", "pry", "pug", "rut", "wad", "wok", "yam", "yen", "yin", "zap", "zen", "zit",
        "pep", "pod", "tux", "ahoy", "bvt", "bal", "zoa", "zea", "wahl", "yon", "foe", "pep",
        "wow", "huh", "hmm", "err", "umm", "ugh",
    ];
