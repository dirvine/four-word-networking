...
fn main() {
    ...
    for (size, bits, name) in &dict_sizes {
        ...
        println!(
            "- {} dictionary ({} bits/word): {} words = {} bits ({:.1}% efficient)",
    ...
    for (_size, bits, name) in &dict_sizes {
    ...
    println!(
        "- Localhost (127.x.x.x): 2^8 addresses = {} (0.000006% of IPv4)",
        256
    );
    println!(
        "- Private 192.168.x.x: 2^16 addresses = {} (0.0015% of IPv4)",
        65536
    );
    println!(
        "- Private 10.x.x.x: 2^24 addresses = {} (0.39% of IPv4)",
        16777216
    );
    println!(
