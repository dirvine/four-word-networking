...
fn main() {
    ...
    for (multiaddr, description) in test_cases {
        ...
        println!(
            "   Compression: {:.1}%",
            (multiaddr.len() as f64 - ultra_compressed.len() as f64) / multiaddr.len() as f64
                * 100.0
        );
        ...
        if ultra_compressed.len() <= 5 {
            match encoder.encode(&ultra_compressed) {
                Ok(encoded) => {
                    ...
                    println!(
                        "   🎯 Result: {:.1}% compression + 4 words total",
                        (multiaddr.len() as f64 - ultra_compressed.len() as f64)
                            / multiaddr.len() as f64
                            * 100.0
                    );
                }
                Err(e) => {
            ...
            match encoder.encode(&ultra_compressed) {
                Ok(encoded) => {
                    let word_count = match &encoded {
                        ...
                        four_word_networking::encoder16k::Encoding16K::Hybrid {
                            digits, ..
                        } => 3 + digits.len() * 4,
                    };
                    println!(
                        "   📈 Encoded: {} ({} total components)",
                        encoded, word_count
                    );
                }
                Err(e) => {
