...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if quality_words.len() < 8192 {
        println!("\n⚠️  Not enough quality words for half-dictionary (need 8192, got {})", quality_words.len());
    ...
    if quality_words.len() >= 8192 {
        ...
        println!("   This will require updating the encoder to use 13-bit indices instead of 14-bit");
    } else {
...
fn is_quality_english_word(word: &str) -> bool {
    ...
    let bad_words = [
        "bvt", "bok", "duh", "duo", "emu", "fax", "gab", "gag", "keg", "nag",
        "dab", "jab", "yon", "zea", "zoa", "wahl", "bal", "oaf", "pox", "pry",
        "pug", "rut", "wad", "wok", "yam", "yen", "yin", "zap", "zen", "zit",
        "pep", "pod", "tux", "sly", "pep", "ahoy"
    ];
    ...
    let good_three_letter = [
        ...
        "did", "car", "eat", "eye", "far", "fun", "got", "hot", "let", "man",
        "mom", "run", "sit", "top", "try", "way", "win", "yes", "big", "red",
        "bad", "cat", "dog", "end", "few", "got", "job", "lot", "put", "say",
        "too", "use", "war", "win", "yet", "zoo", "act", "add", "age", "ago",
        "air", "ask", "bag", "bar", "bed", "bee", "bit", "box", "buy", "cry",
        "cup", "cut", "die", "egg", "fit", "fly", "hat", "hit", "ice", "key",
        "lay", "leg", "lie", "map", "mix", "pay", "pen", "pot", "row", "sea",
        "set", "six", "sun", "tea", "ten", "tie", "toy", "van", "wet", "why"
    ];
...
fn is_lenient_quality_word(word: &str) -> bool {
    ...
    let very_bad_words = [
        "bvt", "zoa", "wahl", "duh", "gab", "gag", "nag", "pep", "pox", "tux",
        "wok", "yin", "zit", "ahoy"
    ];
    ...
    if word.len() == 3 && word.chars().collect::<Vec<_>>().windows(2).any(|w| w[0] == w[1]) {
