...
use crate::{
    compression::{CompressedAddress, IpCompressor},
    ...
    pub fn encode(&self, address: &str) -> Result<String, FourWordError> {
        ...
        if packed.len() > 5 {
            return Err(FourWordError::InvalidInput(format!(
                "Compressed address requires {} bytes, exceeds 5-byte limit for four words",
                packed.len()
            )));
        }
...
mod tests {
    ...
    fn test_private_network_encoding() {
        ...
        let test_cases = vec!["192.168.1.100:443", "10.0.0.1:22", "172.16.0.1:8080"];
