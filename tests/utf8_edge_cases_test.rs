    ...
    for (input, description) in test_cases {
        ...
        match encoder.encode(input) {
            Ok(encoded) => {
                ...
                assert!(decoded.starts_with('['), "Decoded should be IPv6 format for {}", description);
                assert!(decoded.contains("]:"), "Decoded should contain port separator for {}", description);
            }
            Err(_) => {
...
fn test_valid_ipv6_with_port_after_utf8_fix() {
    ...
    for (input, expected_prefix) in valid_cases {
        let encoded = encoder.encode(input).expect("Should encode valid IPv6 with port");
